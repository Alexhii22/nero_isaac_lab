...
try:
    ...
    get_published_pretrained_checkpoint = None  # older Isaac Lab may not have this
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if args_cli.use_pretrained_checkpoint:
        if get_published_pretrained_checkpoint is None:
            raise ModuleNotFoundError(
                "Isaac Lab pretrained_checkpoint not available. "
                "Disable `--use_pretrained_checkpoint` or use `--checkpoint <path>`."
            )
    ...
    if normalizer is not None:
        print("[Play] policy.pt 导出: 含 observation normalizer，sim2sim 时直接喂原始 60 维观测即可。")
    else:
        print("[Play] policy.pt 导出: 不含 observation normalizer，sim2sim 时同样喂原始 60 维观测（训练时未做归一化）。")
